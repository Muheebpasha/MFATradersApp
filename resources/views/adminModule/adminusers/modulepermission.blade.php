...
<div class="content-wrapper">
  ...
  <!-- /.content -->
  <a href="{{ custom_route('/adminusers/add-user-permission') }}" class="btn btn-primary back-to-top" role="button"
...
<script>
  var csrfToken = $('meta[name="csrf-token"]').attr('content');
      ...
      $(document).ready(function() {
          ...
          $(document).on('click', '.remove_module_name', function () {
            const moduleName = $(this).data('item');
            let id = $(this).data('id');
            ...
            $.ajax({
                url: "{{ custom_route('/adminusers/remove-user-module') }}",
                method: 'PATCH',
                headers: {
                ...
                error: function (xhr) {
                    $('#json-container').append('<div class="card text-white bg-danger p-3 mb-3">Failed to remove module.</div>');
                }
            });
        });
      ...
      function fetchRecords(page) {
          ...
          formData += '&page=' + page;
          let url = baseUrl+'user-permission-list';
      ...
      function populateData(records, response) {
          ...
          if (!Array.isArray(records) || records.length === 0) {
              $('#users-table').append('<tr><td colspan="4" class="text-center">No records found</td></tr>');
              return;
          }
  
          $.each(records, function(index, item) {
              let editUserPermission = baseUrl + 'edit-user-permission/' + item.id;
              
              // Example: item.module_list = "Dashboard,Politicians,Temples,Astrology"
              let moduleListString = item.module_list;
              let moduleArray = moduleListString.split(',');
              ...
              badgeHTML = '';
              moduleArray.forEach(function (module_name, index) {
                  let colorClass = badgeClasses[index % badgeClasses.length];
                  badgeHTML += `
                      <span class="badge ${colorClass} d-inline-flex align-items-center px-3 py-2 me-2 mb-2 rounded-pill" style="font-size: 0.875rem; gap: 0.5rem;">
                        <span>${module_name}</span>
                        <button type="button"
                            class="p-0 border-0 bg-transparent text-white fw-bold remove_module_name"
                            data-item="${module_name}" data-id="${item.id}"
                            style="font-size: 1.1rem; line-height: 1; padding: 0 0.5rem; cursor: pointer;"
                            aria-label="Close">
                            &times;
                        </button>
                    </span>`;
              ...
              let row = '<tr id="user-permission-' + item.id + '">' + // Set unique ID here 
                  '<td>' + (((response.currentPage - 1) * response.itemsPerPage) + index + 1) + '.</td>' +
                  '<td>' + item.admin_email + '</td>' +
                  '<td>' + badgeHTML + '</td>' +
                  '<td class="text-center">' +
                      '<div class="d-inline-flex gap-2">'+
                        '<a href="'+ editUserPermission +'" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></a> &nbsp;' +
                        '<a href="javascript:void(0)" class="btn btn-danger btn-sm delete-user-permission" data-id="'+ item.id +'"><i class="fa fa-trash"></i></a> ' +
                      '</div>'+
                  '</td>' +
                  '</tr>';
    ...
    $(document).on('click', '.delete-user-permission', function () {
        var permissionId = $(this).data('id'); // Get the permission ID from the data-id attribute

        if (confirm('Are you sure you want to delete this user permission?')) {
            // Send DELETE request via AJAX
            $.ajax({
                url: "{{ custom_route('/adminusers/delete-user-permission') }}/" + permissionId,
                type: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
                },
                success: function (response) {
                    ...
                    $('#user-permission-' + permissionId).remove();
